...
#library(data.table)
#n <- 10
#dd <- data.table(w = rnorm(n= n), x = rnorm(n = n), pixel_1 = rnorm(n = n))
#dd[, pixel_2 := 0.3 * pixel_1 + rnorm(n)]
#dd[, y := 5 * x + 3 * pixel_1 + 2 * pixel_2 + rnorm(n)]

#create.formula(outcome.name = "y", input.names = "x", input.patterns = c("pi", "xel"), all.data.names = names(dd), return.as = "character")
