...
create.formula <-
  function(outcome.name,
    ...
    if (is.data.frame(dat)) {
      ...
      inclusion.table <-
        data.table(variable = unique(c(
          input.names, variable.names.from.patterns
        )))
      ...
      if (reduce == TRUE) {
        ...
        inclusion.table <-
          merge(x = inclusion.table, y = min.categories.tab, by = "variable")
    ...
    res <-
      list(formula = the.formula, inclusion.table = inclusion.table)
