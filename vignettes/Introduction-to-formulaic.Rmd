...
```{r stats_image, out.width = "400px", echo = FALSE}
include_graphics("../vignettes/Screen Shot -- Engagement Model with Age Group Terms.png", auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE), dpi = NULL)

include_graphics("../vignettes/Screen Shot -- Engagement Model without Age Group Terms.png", auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE), dpi = NULL)
...
formula.awareness <- create.formula(outcome.name = awareness.name, input.names = c(age.group.name, gender.name))
...
```{r create.formula with lack of contrast 2}

formula.awareness <- create.formula(outcome.name = awareness.name, input.names = c(age.group.name, gender.name), dat = snack.dat[get(age.group.name) == "[ 18, 35)",], reduce = TRUE)
...
formula.consideration.1 <- create.formula(outcome.name = consideration.name, input.names = c(age.group.name, gender.name, awareness.name), dat = snack.dat, reduce = TRUE, max.outcome.categories.to.search = 1)
...
formula.consideration.2 <- create.formula(outcome.name = consideration.name, input.names = c(age.group.name, gender.name, awareness.name), dat = snack.dat, reduce = TRUE, max.outcome.categories.to.search = 2)
...
create.formula(outcome.name = satisfaction.name, input.names = c(age.name, income.name, region.name, id.name), dat = snack.dat, reduce = TRUE, max.input.categories = 30)$formula
```
...
*  max.input.categories This limits the maximum number of variables that will be employeed in the formula. As default it is set at 20, but users can still change at his/her convinence.
*  max.outcome.categories.to.search This limits the maximum number of outcome categories will be investigated in the formula. As default it is set at 4, but users can still change at his/her convinence.
*  force.main.effects This is a logical value.  When TRUE, the intent is that any term included as an interaction (of multiple variables) must also be listed individually as a main effect.
...
reduce.existing.formula(the.initial.formula = the.initial.formula, dat = snack.dat, max.input.categories = 30)$formula
```
